...
- `tool_call/1` — a call the agent made. The name is the point; arguments are not rendered yet.
...
Every `data-test` the spex assert on is part of the contract: `message`, `tool-call`, `tool-result`, `chat-halted`, `chat-failed`, and the marker `notice/1` is given.
