...
- `question/1` — a question the agent asked, answerable where it is shown. Lifted out of `QuestionLive.Show`; that page still renders it, and a transcript can too. Answered questions keep rendering, showing what was asked beside what was said — in a transcript the answer is part of the record, not a modal that closes. Three states, not two: a question the run stopped waiting on is neither answered nor pending, and matching only those two rendered a card with a title and an empty body — a page that reads as broken rather than as a question that timed out. Whatever the status, what was asked is still shown; it is the point of keeping the record.
