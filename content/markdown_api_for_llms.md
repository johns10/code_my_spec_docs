...
## Can you use directories and markdown files to give LLMs project context?
...
## What about using shell scripts that wrap HTTP calls as LLM tools?
...
## How does a web server that returns markdown solve the LLM context problem?
...
## Why does a markdown API work better than stuffing everything into CLAUDE.md?
...
**What is a markdown API for LLMs?** A markdown API is a web server where every endpoint returns markdown instead of JSON. The LLM can navigate it using WebFetch or curl, discovering tools and reading project state through the same interface a human would use. The same markdown can be rendered as HTML for human consumption with minimal conversion.

**Why not just use MCP servers instead of a markdown API?** MCP servers add tools to the context window, and each tool consumes tokens whether or not the agent uses it. A markdown API keeps the context lean because the agent only fetches what it needs on demand. You get progressive disclosure without context proliferation.
