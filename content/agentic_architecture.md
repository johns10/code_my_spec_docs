---
# Metadata for agentic_architecture.md

# Required fields
slug: agentic-architecture
type: blog
title: "Architecture for AI Agents: Which Patterns Actually Work?"
...
# SEO Metadata
meta_title: "Architecture for AI Agents: Which Patterns Work?"
meta_description: "A 26-point quality gap between AI-only code and human-guided architecture. Here's which patterns produce the best agent output, ranked."
og_title: "Architecture for AI Agents: Which Patterns Actually Work?"
og_description: "Modular monoliths, vertical slices, hexagonal, microservices - ranked by how well AI agents perform in each. With evidence."
og_image: null
...
Every architecture should be evaluated by one question: does it fit in the agent's context window? The architecture you choose determines how much of your codebase the agent must load to make a single change. If it has to load too much, the quality of its output degrades. The degradation is measurable. Sebastian Sigl [puts it well](https://www.thecodecoach.dev/blog/bounded-contexts-are-the-key-to-effective-ai-driven-development): "The problem is that we give it too much irrelevant context, which creates ambiguity. And ambiguity is where correctness usually dies." The [40% rule](https://dev.to/somedood/vertical-slice-architecture-is-the-new-meta-for-agentic-coding-hma) quantifies this: LLMs degrade past 40% context usage, so if your architecture forces the agent to load half the codebase to make a change, you're already in the degradation zone. The architecture must create bounded scope, clear contracts, and predictable conventions so the agent can work within a focused context window without loading irrelevant code that degrades its reasoning.
...
The most interesting development isn't a specific pattern. It's the idea that architecture itself becomes machine-readable context that the agent can consume. Instead of relying on the agent to infer your architectural decisions from code alone, you encode those decisions as structured documents that the agent reads before making changes. [Archgate](https://www.infoq.com/news/2025/12/archgate-architecture-decision-records/) turns Architecture Decision Records into executable TypeScript rules that enforce decisions in CI and feed context to agents automatically. When an ADR says "all API endpoints must use the shared response format," Archgate returns file-and-line violations. This transforms architecture from a passive constraint into an active input that shapes every generation.

Two emerging approaches push this further. The [Steward Agents concept](https://blog.thinktastic.com/why-domain-driven-design-is-having-a-renaissance-in-the-age-of-ai-agents) assigns one agent per bounded context, where each agent maintains its own documentation, ADRs, and internal conventions, becoming the domain expert for its context. An [arXiv paper on "Codified Context"](https://arxiv.org/abs/2603.05344) documents a three-tier system: hot-memory constitution, 19 domain-expert agents, and 34 on-demand spec documents, tested across 283 sessions on a 108K-line system. The conclusion from both approaches is the same: single-file manifests like CLAUDE.md don't scale past modest codebases, and you need structured, layered architecture context that matches your domain boundaries.

{{verification_gate_cta shape="aside" headline="Human guided architecture scored 85 out of 105 in that study, and AI only code scored 59." sub="Architecture is where the entire gap lives, and documented conventions do not close it." body="CodeMySpec encodes the contract as a spec the build enforces, then a QA agent drives the running app to check it." label="See how the build enforces specs" campaign="agentic-architecture"}}

## What Should You Do First?

If you're starting a new project with AI agents, pick a modular monolith with bounded contexts or vertical slices. These patterns give the agent the smallest possible scope for each change while maintaining a single codebase. Enforce boundaries with linters, not documentation, because documented conventions get ignored, but linter failures block merges. Give the agent architecture context it can read (ADRs, component specs, design documents), not just code. If you have an existing codebase, the first investment isn't better prompts. It's better boundaries. Even adding one enforced boundary between two domains improves agent output measurably. The architecture is the most neglected phase in the agentic development process, and it's also the highest leverage: a 26-point quality gap from architecture alone tells you everything about where to invest your time.
