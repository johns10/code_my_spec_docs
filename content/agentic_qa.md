...
# SEO Metadata
meta_title: "Agentic QA - AI-Powered Application Testing - CodeMySpec"
meta_description: Unit tests and BDD specs verify pieces. QA verifies the running application — story QA, journey QA, and automated issue filing by AI agents.
og_title: "Agentic QA"
og_description: "An AI agent tests every story against the running app, finds bugs unit tests miss, and files issues automatically. Over 100 issues caught on Fuellytics."
...
- **SMS delivery tracking failed for new transactions.** The webhook returned `sms_message_sid: "pending"` (hardcoded) instead of the real Twilio SID. Delivery status callbacks couldn't find the message. The tracking feature existed and worked, but nobody had wired it up to new transactions.

- **A fraud surface in the verification flow.** Yellow-flagged transactions (driver didn't respond within 6 minutes) cleared their flag when the driver *opened* the verification link. No photo required. BDD specs tested that the flag cleared after "verification." QA revealed that visiting the URL was enough. A flagged driver could clear the flag by tapping a link.
...
Unit tests would have missed all these. The BDD spec for the fraud vulnerability *explicitly passed*, because the spec's definition of "verify" was too loose.
