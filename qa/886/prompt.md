# QA Story 886: The preview points at a real app, over a URL that actually works
...
As Sam, I want the preview pane to be showing my app really running, so that what I am looking at is the thing that was built rather than a placeholder.

## Acceptance criteria

- The pane shows the running app itself
- An app still coming up says so, and changes over on its own
- An app answering with errors is broken, not running
- The framed app keeps its session
- A change in the app reaches the pane without a reload
- A crash in the preview becomes a signal, not a blank frame
- Outside the preview the bridge does nothing, and answers no one
- The address Sam shared yesterday still points at his app today
...
Write the brief to `.code_my_spec/qa/886/brief.md` matching this spec exactly.
