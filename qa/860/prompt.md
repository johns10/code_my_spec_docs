# QA Story 860: I can receive mail at my domain
...
Sam can receive email sent to his domain, not only send from it.

## Acceptance criteria

- A message to Sam's domain arrives where he reads mail
- Receiving mail does not break sending it
- The inbox is proven by a message, not by configuration
- The app can act on a message someone sent in
- Sam names the address his mail reaches him at
- A domain that receives no mail today gets its inbound records published
- A domain already receiving mail elsewhere is not silently redirected

## BDD spec files

- `test/spex/974_i_can_receive_mail_at_my_domain/criterion_8030_a_message_to_sams_domain_arrives_where_he_reads_mail_spex.exs`
- `test/spex/974_i_can_receive_mail_at_my_domain/criterion_8032_receiving_mail_does_not_break_sending_it_spex.exs`
- `test/spex/974_i_can_receive_mail_at_my_domain/criterion_8033_the_inbox_is_proven_by_a_message_not_by_configuration_spex.exs`
- `test/spex/974_i_can_receive_mail_at_my_domain/criterion_8048_the_app_can_act_on_a_message_someone_sent_in_spex.exs`
...
Write the brief to `.code_my_spec/qa/860/brief.md` matching this spec exactly.
