# QA Story 822: Architect Agent Surface
...
As the architect agent (Claude executing an architecture session), I need the right tools registered, the right prompt content, and tools that resolve scope from the local working directory — so that I can perform the architect's design work without silent degradation when the prompt names an unreachable tool or when a tool fails on an under-populated session scope.

This story is the agent-perspective companion to "Architecture Design" (story 70). Story 70 captures human-architect outcomes (orphans resolved, stories linked, proposal valid). This story captures agent-side mechanics: tool-surface registration, prompt-as-contract correctness, and scope-resolution robustness.

## Acceptance criteria

- Architecture prompt only references tools registered on LocalServer
- Architect patches existing architecture without redesigning it
- Architect reuses an existing component to satisfy a new story
- Architect links a story to a component via the component linker tool
- Architect creates a stub component via the create_component tool
- Architect rewrites proposal in patch mode and triggers execution manually

## BDD spec files

- `test/spex/682_architect_agent_surface/criterion_5829_architect_patches_existing_architecture_without_redesigning_it_spex.exs`
- `test/spex/682_architect_agent_surface/criterion_5830_architect_reuses_an_existing_component_to_satisfy_a_new_story_spex.exs`
- `test/spex/682_architect_agent_surface/criterion_5831_architect_links_a_story_to_a_component_via_the_component_linker_tool_spex.exs`
- `test/spex/682_architect_agent_surface/criterion_5832_architect_creates_a_stub_component_via_the_create_component_tool_spex.exs`
- `test/spex/682_architect_agent_surface/criterion_5833_architect_rewrites_proposal_in_patch_mode_and_triggers_execution_manually_spex.exs`
- `test/spex/682_architect_agent_surface/criterion_5874_architecture_prompt_only_references_tools_registered_on_local_server_spex.exs`
...
Write the brief to `.code_my_spec/qa/822/brief.md` matching this spec exactly.
