# QA Story 795: BDD Specification Lifecycle
...
As a developer, I want executable BDD specifications written for each user story so that acceptance criteria are automatically verified.

## Acceptance criteria

- Three-criterion story produces three spec files at the harness-assigned paths
- Missing spec file is reported with the criterion id and expected path
- Specs that exist, parse, have scenarios with steps, and compile return :valid
- Spec file with no scenarios produces the "Must contain at least one scenario" reason
- Scenario without Given/When/Then steps produces the "All scenarios must have Given/When/Then steps" reason
- Compilation failure produces a diagnostic listing each error's file and line
- Every component type's prompt scaffold opens with `use <App>Spex.Case` as the only test-case `use`
- Spec written with ConnCase instead of `<App>Spex.Case` fails to compile on the undefined `spex` macro

## BDD spec files

- `test/spex/460_bdd_specification_lifecycle/criterion_5530_three-criterion_story_produces_three_spec_files_at_the_harness-assigned_paths_spex.exs`
- `test/spex/460_bdd_specification_lifecycle/criterion_5531_missing_spec_file_is_reported_with_the_criterion_id_and_expected_path_spex.exs`
- `test/spex/460_bdd_specification_lifecycle/criterion_5536_specs_that_exist_parse_have_scenarios_with_steps_and_compile_return_valid_spex.exs`
...
- `test/spex/460_bdd_specification_lifecycle/criterion_5539_compilation_failure_produces_a_diagnostic_listing_each_errors_file_and_line_spex.exs`
- `test/spex/460_bdd_specification_lifecycle/criterion_5556_every_component_types_prompt_scaffold_opens_with_use_app_spex_case_as_the_only_test_case_use_spex.exs`
- `test/spex/460_bdd_specification_lifecycle/criterion_5557_spec_written_with_conncase_instead_of_spex_case_fails_to_compile_on_undefined_spex_macro_spex.exs`
...
Write the brief to `.code_my_spec/qa/795/brief.md` matching this spec exactly.
