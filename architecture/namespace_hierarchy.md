CodeMySpec [module]
├── AcceptanceCriteria [module] Phoenix context for managing acceptance criteria as first-class entities. Acceptance criteria belong to stories and r...
...
│   ├── DevelopLiveView [module] Orchestrates the full lifecycle of a LiveView from specification through implementation. Mirrors DevelopContext's pha...
...
│   ├── QaApp [module] Orchestrator task for browser-based QA of the running app. Analyzes the project's routes, stories, and BDD spec cover...
│   ├── QaStory [module] Per-story agent task for QA-testing a single user story. Takes a story ID, loads the story (title, description, accep...
...
├── BddSpecs [module] Context for working with BDD specifications. Provides file-based discovery and coverage analysis of Spex DSL specs wi...
...
├── ClientUsers [module]
│   └── ClientUser [module]
├── Code [module] Provides AST operations on code files for extracting structured information like dependencies, functions, and test as...
...
├── Documents [module] Context for managing document parsing, validation, and creation from markdown content. Provides a centralized registr...
...
├── Encrypted
│   └── Binary [module]
├── Environments [module] Provides execution primitives for running commands and file system operations across different execution contexts (se...
...
│   └── Application [module]
├── Paths [module]
├── Problems [module] The Problems context provides a unified abstraction for problems, warnings, and errors discovered across different an...
...
├── Projects [module]
│   └── Project [module]
├── Quality [module] Provides abstractions and functionality for checking quality of Elixir code and test files including compilation vali...
...
├── Sessions [module] Manages agent task sessions backed by the filesystem. Each session is a directory under `.code_my_spec/internal/sessi...
...
├── StaticAnalysis [module] Executes optional code quality and correctness tools against a project codebase. Provides a unified interface for run...
...
├── Stories [module] Phoenix context for managing user stories within projects. Provides the public API for story CRUD operations, compone...
...
├── Tags [module] Simple tagging system for organizing stories. Tags are strings scoped to a project, and stories can have multiple tags.
│   ├── StoryTag [module]
│   ├── Tag [module]
...
├── Validation [module] Validates files edited during Claude Code sessions. Parses transcripts to extract edited files, runs the generic vali...
...
├── ProjectCoordinatorController [module]
├── ProjectCoordinatorJSON [module]
├── ProjectLive
│   ├── Components
